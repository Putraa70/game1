...
public class Controller implements EventHandler<KeyEvent> {
    ...
    private static final String[] levelFiles = {"src/level/level1.txt", "src/level/level2.txt", "src/level/level3.txt"};
    ...
    if (this.pacManModel == null) {
        String file = Controller.getLevelFile(0); // Ambil level file
        this.pacManModel = new PacManModel(); // Inisialisasi PacManModel
    }

    // Pastikan Pac-Man hanya diciptakan sekali
    if (this.pacManModel.getPacman() == null) {
        Hero pacManModel1 = new Hero(1, 20, Direction.NONE); // Membuat Pac-Man
        this.pacManModel.setPacman(pacManModel1); // Set Pac-Man ke dalam PacManModel
    }

    this.update(PacManModel.Direction.NONE); // Perbarui permainan dengan arah NONE
    ...
    private void update(PacManModel.Direction direction) {
        ...
        Hero pacman = this.pacManModel.getPacman();
        this.pacManView.updatePacman(pacman.getX(), pacman.getY(), pacman.getDirection());
        
         // Perbarui tampilan Ghosts
        for (Enemy ghost : this.pacManModel.getGhosts()) {
            this.pacManView.updateGhost(ghost.getX(), ghost.getY(), ghost.getDirection());
        }

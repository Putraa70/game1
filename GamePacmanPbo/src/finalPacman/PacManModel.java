...
public class PacManModel {
    ...
    private final ArrayList<Object> enemies;
    ...
    private Hero pacman; // Objek Pac-Man
    private List<Enemy> ghosts; // Daftar Ghosts
    /**
     * Start a new game upon initializion
     */
// Pac-Man dan musuh diatur dalam model, hindari inisialisasi di tempat lain


    public PacManModel(int rowCount, int columnCount) {
        this.rowCount = rowCount;
        this.columnCount = columnCount;
        // Inisialisasi pacman dan enemies
        this.pacman = new Hero(0, 0, Direction.NONE);
    ...
    try (Scanner scanner = new Scanner(file)) {
        ...
        while (scanner.hasNextLine()) {
            ...
            for (int column = 0; column < line.length(); column++) {
                ...
                if (value == 'W') {
                    ...
                    ghosts.add(new Enemy(row, column, Direction.LEFT)); // Tambahkan Ghost pertama
                } else if (value == '2') {
                    thisValue = CellValue.GHOST2HOME;
                    ghosts.add(new Enemy(row, column, Direction.RIGHT)); // Tambahkan Ghost kedua
                } else if (value == 'P') {
                    thisValue = CellValue.PACMANHOME;
                    pacman = new Hero(row, column, Direction.NONE); // Atur posisi PacMan
                } else {
        ...
        System.err.println("Error: File level tidak ditemukan!");
    }
}
    ...
    public void startNewGame() {
        ...
        // Panggil initializeLevel untuk memuat level awal
        initializeLevel("level1.txt"); // Ganti dengan level awal yang diinginkan
    }
...
//        pacman.move(this); // Gerakkan Pac-Man

        // Gerakkan setiap Ghost
    ...
    public List<Enemy> getGhosts() {
        return ghosts;
    }
     // Menambahkan metode setPacman
    public void setPacman(Hero pacman) {
        this.pacman = pacman;
    }
}
